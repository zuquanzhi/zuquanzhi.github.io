...
description: ROS1 中 Topic、Service、Param、Action 四种通信机制的 C++ 与 Python 实现整理。
keywords:
  - ROS1
  - Topic
  - Service
...
1. 话题 Topic 是节点间进行持续通信的发布-订阅机制。
2. 通信双方利用话题名称建立连接，话题中传输的数据单元称为消息（Message）。
3. 消息通常按固定频率持续发送以保证实时性。
4. 消息发送方为发布者（Publisher），接收方为订阅者（Subscriber）。

**更多规则：**

1. 一个 ROS 节点网络中可同时存在多个话题。
2. 一个话题可以有多个发布者和多个订阅者。
3. 一个节点可同时订阅或发布多个话题。
4. 不同传感器通常各自使用独立话题名称，每条话题只有一个发布者。
5. 速度指令话题也可以有多个发布者，但同一时刻只有一个生效。
...
1. 确定话题名称和消息类型。
2. `include` 消息类型对应的头文件。
3. 通过 `NodeHandle` 发布话题，得到 `Publisher` 对象。
4. 构造消息包并赋值。
5. 调用 `pub.publish(msg)` 发送消息。

**常用调试命令：**

```bash
rostopic list              # 列出所有活跃话题
rostopic echo <话题名>     # 打印话题消息内容
rostopic hz <话题名>       # 统计话题发送频率
```

**订阅者流程：**

1. 确定话题名称和消息类型。
2. `include` `ros.h` 以及消息类型对应头文件。
3. 通过 `NodeHandle` 订阅话题，设置回调函数。
4. 定义回调函数，处理接收到的消息包。
5. `main` 中调用 `ros::spinOnce()` 或 `ros::spin()` 驱动回调触发。
...
运行三个节点和 roscore 后，在新终端执行：

```bash
rqt_graph
```

可以打开可视化界面，观察当前节点间的消息链路。

![](https://cdn.nlark.com/yuque/0/2024/png/39228163/1705307614453-48eb5146-7ae3-40e1-b12c-6cc0a99ed5cb.png)

### launch 文件同时启动多个节点

launch 文件是遵循 XML 语法的 ROS 启动描述文件，通过 `roslaunch` 可一次启动多个节点。

**常用属性说明：**

- `output="screen"`：将该节点的日志输出到终端（注意：`ROS_WARN` 不受此属性控制）。
- `launch-prefix="gnome-terminal -e"`：让该节点在独立终端窗口中运行。
...
Python 实现思路与 C++ 基本一致，区别在于用 `rospy` 替代 `NodeHandle` 做初始化和发布/订阅操作。
...
if __name__ == "__main__": #主函数
    ...
    rospy.loginfo("node_chao: node started")
...
if __name__ == "__main__":
    ...
    rospy.spin()
```
  两种语言的实现思路几乎一致，只是在 launch 文件中有细微差别：C++ 节点 `type` 填写可执行文件名，Python 节点 `type` 需要加 `.py` 后缀。
```xml
<launch>
...
Service 是 ROS 中的请求-响应式通信机制，适合需要明确返回结果的场景（与话题的单向持续发送不同）。

**通信流程：**

1. 服务端（Server）注册服务并等待请求。
2. 客户端（Client）发起调用。
3. 节点管理器完成服务地址匹配。
4. 客户端发送请求，等待服务端完成处理。
5. 服务端返回响应结果。
...
打开图形化 service 调用界面，按界面提示填写参数并发送请求，可以看到仿真界面中新增了一只小乌龟。

![](https://cdn.nlark.com/yuque/0/2024/png/39228163/1705388668505-d518fb03-f941-4f5c-b0f6-7f08d92ac516.png)

![](https://cdn.nlark.com/yuque/0/2024/png/39228163/1705388704513-fa622c00-39a1-498e-85e3-d908b340bc3e.png)
...
这段客户端代码用类封装，循环发起两个数相加的服务请求。核心接口：
```xml
