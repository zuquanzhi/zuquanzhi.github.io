...
举个例子。如果 AI 要帮你发一封邮件，它需要读你的邮件内容、获取联系人权限、理解上下文。你可以通过 spec 限定它"只能发这封邮件"，但它实际能接触到的信息远不止这封邮件——你的收件箱、过往联系人、三个月前的往来记录，全在上下文窗口里。

问题不在于权限设没设好。问题在于：你不可能提前穷举所有"不能用"的情况。AI 看到了三个月前的某封邮件，在你不知情的情况下把里面的信息用在了当前任务里——你可以说这是越权，但你没办法在一开始就写清楚"不许引用三个月前的邮件"这种规则。

约束体系的假设是你能把边界说清楚。但现实中大量边界本身就是模糊的，只能在出问题之后补一句"这个不算"。规则写得再多也没用，这种形式本身就覆盖不了所有模糊边界。

**第二个问题：代码 review。**

代码 review 这件事，AI 能做到什么程度？

review 不只是检查语法和风格，还包括判断"这段设计是否符合团队的技术选型"、"这个实现是否埋了潜在的地雷"、"这个改动会不会影响其他模块的兼容性"——这些东西靠规则解决不了，靠的是判断力。
...
这是信息的根本性问题，模型再强也绕不过去。

### 2. 推理能力有上限

即便输入信息完整，模型的推理能力也不是无限的。长上下文会稀释关键信息，多跳推理依然困难重重，复杂的因果链条依然难以准确追踪。

你让它写一个两百行的工具函数，它写得很利索。你让它设计一个跨越五个模块的系统架构，每一步推导都依赖上一步的正确性——偏差就开始累积。第一步偏一点，第五步完全跑偏。

**问题不在提示词写没写到位，推理深度本身有它的天花板。**

### 3. 执行能力有瓶颈

AI 能生成方案，但方案到执行之间隔着无数的细节。

一段能跑的代码，到能上线的服务，中间还有部署、监控、回滚、容错。现在的 Agent 确实能帮你跑 CI、查日志、改配置，比以前强太多了。但它做的事本质上是"哪里报错修哪里"，而不是"上线前就预判到三个月后流量翻十倍会崩在哪个接口"。执行这件事，难的从来不是修复已知的错，而是预判还没发生的问题。
...
*美团这轮面试后来没有后续。但这两个问题我到现在还在琢磨，索性写出来。*