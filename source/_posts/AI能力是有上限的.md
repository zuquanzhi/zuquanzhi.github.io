...
问题不在于权限设没设好。问题在于：你不可能提前穷举所有"不能用"的情况。AI 看到了三个月前的某封邮件，在你不知情的情况下把里面的信息用在了当前任务里——你可以说这是越权，但你没办在一开始就写清楚"不许引用三个月前的邮件"这种规则。

约束体系的假设是你能把边界说清楚。但现实中大量边界本身就是模糊的，只能在出问题之后补一句"这个不算"。这不是规则写得不够多，是规则这种形式本身就覆盖不了所有模糊边界。
...
**这不是"提示词没写到位"，而是推理深度本身有它的天花板。**

### 3. 执行能力有瓶颈

AI 能生成方案，但方案到执行之间隔着无数的细节。

一段能跑的代码，到能上线的服务，中间还有部署、监控、回滚、容错。现在的 Agent 确实能帮你跑 CI、查日志、改配置，比以前强太多了。但它做的事本质上是"哪里报错修哪里"，而不是"上线前就预判到三个月后流量翻十倍会崩在哪个接口"。执行这件事，最难的不是修复，是预判。
